...
-- 5) Enable Row Level Security and add permissive policies (until auth is added)
alter table public.contract enable row level security;
alter table public.contact enable row level security;

-- Contracts policies
create policy if not exists "Public can read contracts"
  on public.contract for select
  using (true);

create policy if not exists "Public can insert contracts"
  on public.contract for insert
  with check (true);

create policy if not exists "Public can update contracts"
  on public.contract for update
  using (true)
  with check (true);

create policy if not exists "Public can delete contracts"
...
create policy if not exists "Public can insert contacts"
  on public.contact for insert
  with check (true);

create policy if not exists "Public can update contacts"
  on public.contact for update
  using (true)
  with check (true);

create policy if not exists "Public can delete contacts"
